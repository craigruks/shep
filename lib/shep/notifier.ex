defmodule Shep.Notifier do
  @moduledoc "Slack webhook notifications for task failures and stalls."

  require Logger

  @doc "Notify on task failure."
  @spec notify_failure(Shep.Task.t(), String.t()) :: :ok
  def notify_failure(%Shep.Task{} = task, reason) do
    post_slack(%{
      text: ":x: Shep task #{task.id} failed",
      blocks: [
  ...
  def notify_stall(task_id, idle_ms) do
    ...
    post_slack(%{
      text: ":warning: Shep task #{task_id} stalled (#{minutes}min idle)"
    })
  end

  defp post_slack(payload) do
    url = Application.get_env(:shep, :slack_webhook_url)
