defmodule Factory.Orchestrator do
  ...
  alias Factory.Orchestrator.Dispatch

  @ets_table :factory_state
  ...
  @spec submit(Factory.Task.t()) :: :ok | {:error, String.t()}
  def submit(%Factory.Task{} = task) do
  ...
  def handle_call({:pause, task_id}, _from, state) do
    case Map.get(state.running, task_id) do
      ...
      entry ->
        paused_task = %Factory.PausedTask{
          task: entry.task,
  ...
  defp tick(state) do
    ...
    case Factory.Tracker.fetch_candidates() do
      {:ok, tasks} ->
  ...
  defp deps_resolved?(repo, %{depends_on: deps}) do
    Factory.Tracker.GitHub.deps_resolved?(repo, deps)
  end
    ...
    case Map.get(state.running, task_id) do
      %{last_output_at: last} when is_integer(last) ->
        ...
        if idle_ms > idle_timeout do
          ...
          Factory.Notifier.notify_stall(task_id, idle_ms)
          kill_task(task_id, state)
        end
