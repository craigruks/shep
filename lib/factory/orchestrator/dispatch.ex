defmodule Factory.Orchestrator.Dispatch do
  ...
  @spec dispatch_task(Factory.Task.t(), struct()) :: struct()
  ...
  @spec dispatch_resume(Factory.PausedTask.t(), struct()) :: struct()
  def dispatch_resume(%Factory.PausedTask{} = paused_task, state) do
    config = Factory.Config.current!()
    task = paused_task.task

    %{pid: pid, ref: ref} =
      Task.Supervisor.async_nolink(Factory.TaskSupervisor, fn ->
        Factory.AgentRunner.run(task, self(), %{
          config: config,
  ...
  defp do_dispatch(task, config, state) do
    ...
    %{pid: pid, ref: ref} =
      Task.Supervisor.async_nolink(Factory.TaskSupervisor, fn ->
        Factory.Tracker.claim(task.id)
        Factory.AgentRunner.run(task, self(), %{config: config})
      end)
