defmodule Factory.Hooks do
  ...
  def run(command, cwd, opts) when is_binary(command) and is_binary(cwd) do
    ...
    case Task.yield(task, timeout) || Task.shutdown(task) do
      {:ok, {_output, 0}} ->
        :telemetry.execute([:factory, :hook, :stop], %{}, %{hook: hook_name, exit_code: 0})
        :ok

      {:ok, {output, code}} ->
        Logger.warning("Hook #{hook_name} exited #{code}: #{String.trim(output)}")
        :telemetry.execute([:factory, :hook, :stop], %{}, %{hook: hook_name, exit_code: code})
        {:error, "hook exited #{code}"}

      nil ->
        Logger.warning("Hook #{hook_name} timed out after #{timeout}ms")
        :telemetry.execute([:factory, :hook, :stop], %{}, %{hook: hook_name, exit_code: :timeout})
