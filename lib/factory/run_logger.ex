defmodule Factory.RunLogger do
  ...
  def attach do
    events = [
      [:factory, :agent, :start],
      [:factory, :agent, :stop],
      [:factory, :agent, :stdout],
      [:factory, :orchestrator, :dispatch],
      [:factory, :hook, :start],
      [:factory, :hook, :stop]
    ]

    :telemetry.attach_many("factory-jsonl-logger", events, &__MODULE__.handle_event/4, :jsonl)
    :telemetry.attach_many("factory-axiom-logger", events, &__MODULE__.handle_event/4, :axiom)

    :telemetry.attach_many(
      "factory-stdout-logger",
      [[:factory, :agent, :stdout], [:factory, :agent, :stop]],
  ...
  @spec detach() :: :ok
  def detach do
    :telemetry.detach("factory-jsonl-logger")
    :telemetry.detach("factory-axiom-logger")
    :telemetry.detach("factory-stdout-logger")
    :ok
  end
  ...
  def handle_event([:factory, :agent, :stdout], _measurements, metadata, :stdout) do
    write_stdout(metadata)
  end

  def handle_event([:factory, :agent, :stop], measurements, metadata, :stdout) do
  ...
  defp write_stdout_marker(%{task_id: task_id, completion: completion}, %{duration_ms: ms})
    ...
    status =
      case completion do
        %Factory.Completion.Failed{} -> "FAILED"
        _ -> "COMPLETE"
      end
  ...
  defp emit_axiom(event, measurements, metadata) do
    token = Application.get_env(:factory, :axiom_token)
    ...
    Task.start(fn ->
      Req.post("https://api.axiom.co/v1/datasets/lk-event-logs/ingest",
        json: [payload],
