defmodule Factory.WorktreeTest do
  use ExUnit.Case

  alias Factory.Worktree

  describe "list/1" do
    test "returns empty list for nonexistent directory" do
      assert [] == Worktree.list("/tmp/factory_test_nonexistent_#{System.unique_integer()}")
    end

    test "returns directories in root" do
      root = Path.join(System.tmp_dir!(), "factory_wt_test_#{System.unique_integer([:positive])}")
  ...
  describe "create/3 handles stale state" do
    test "cleanup_stale removes leftover directory before creating worktree" do
      root =
        Path.join(System.tmp_dir!(), "factory_stale_test_#{System.unique_integer([:positive])}")
    ...
    test "sanitize_branch replaces non-alphanumeric chars but keeps hyphens" do
      ...
      assert "factory_49" == Worktree.sanitize_branch_for_test("factory/49")
    end
  end
end
