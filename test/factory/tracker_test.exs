defmodule Factory.Tracker.GitHubTest do
  use ExUnit.Case, async: true

  alias Factory.Tracker.GitHub

  describe "parse_task_type/1" do
    test "extracts type from type: label" do
      labels = [%{"name" => "factory"}, %{"name" => "type:lint-fix"}]
  ...
  describe "no_merge?/1" do
    test "true when factory:no-merge label present" do
      labels = [
        %{"name" => "factory"},
        %{"name" => "factory:no-merge"},
        %{"name" => "type:lint-fix"}
      ]
...
defmodule Factory.Tracker.MemoryTest do
  use ExUnit.Case, async: false

  alias Factory.Tracker.Memory
  ...
  test "add_task and fetch" do
    task = %Factory.Task{id: "1", branch: "test/1", prompt: "hello"}
