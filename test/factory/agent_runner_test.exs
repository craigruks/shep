defmodule Factory.AgentRunnerTest do
  use ExUnit.Case, async: true

  alias Factory.AgentRunner
  ...
  describe "Claude.build_args/2" do
    ...
    test "includes --name with session name" do
      ...
      assert name_idx != nil
      assert Enum.at(args, name_idx + 1) == "factory-99"
    end
  end
  ...
  describe "Claude.build_resume_args/1" do
    ...
    test "includes --name with session name" do
      ...
      name_idx = Enum.find_index(args, &(&1 == "--name"))
      assert name_idx != nil
      assert Enum.at(args, name_idx + 1) == "factory-42"
    end
  ...
  describe "completion parsing from stream-json" do
    test "extracts completion from assistant message JSON" do
      ...
      assert %Factory.Completion.Complete{summary: "fixed lint"} = completion
    end
    ...
    test "extracts completion from result JSON" do
      ...
      assert %Factory.Completion.Failed{reason: "cannot fix"} = completion
    end
